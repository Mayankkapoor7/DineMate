...
class Database:
    ...
    def store_order_db(self, order_dict: Dict[str, int], price: float, status: str = "Pending", username: Optional[str] = None) -> Optional[int]:
        """📝 Store a new order.
        ...
        Args:
            ...
            username (Optional[str]): Username of the customer.
        ...
        try:
            ...
            self.cursor.execute(
                """
                INSERT INTO orders (id, items, total_price, status, date, time, username)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (order_id, json.dumps(order_dict), price, status, now.strftime("%Y-%m-%d"), now.strftime("%I:%M:%S %p"), username)
            )
    ...
    def get_last_user_order(self, username: Optional[str] = None) -> Optional[Dict]:
        """Fetch the user's most recent confirmed (non-canceled) order."""
        try:
            if username:
                self.cursor.execute(
                    """
                    ...
                    FROM orders
                    WHERE username = ? AND status != 'Canceled'
                    ORDER BY id DESC
                    LIMIT 1
                    """,
                    (username,)
...
class AsyncDatabase:
    ...
    async def store_order_db(self, order_dict: Dict[str, int], price: float, status: str = "Pending", username: Optional[str] = None) -> Optional[int]:
    ...
    async def get_last_user_order(self, username: Optional[str] = None) -> Optional[Dict]:
        """Fetch the user's most recent confirmed (non-canceled) order (Async)."""
        try:
            if username:
                await self.cursor.execute(
                    """
            ...
            logger.error({"error": str(e), "message": "❌ Error fetching last user order (Async)"})
            return None
